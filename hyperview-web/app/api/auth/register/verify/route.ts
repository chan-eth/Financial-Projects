...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!ceremonyId) {
    return NextResponse.json(
      { error: "registration ceremony cookie missing or expired" },
      { status: 400 },
    );
  }
  ...
  try {
    body = (await request.json()) as { response?: unknown; deviceLabel?: string };
  } catch {
    return NextResponse.json({ error: "invalid json body" }, { status: 400 });
  }

  const res = await workerPost("/auth/passkey/register/verify", {
    ceremonyId,
    response: body.response,
    deviceLabel: body.deviceLabel,
  });

  // Clear the ceremony cookie regardless of outcome — it's one-shot.
